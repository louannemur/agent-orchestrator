...
export async function logsCommand(agentId, options) {
    ...
    async function fetchAndDisplayLogs(lastLogId) {
    ...
    try {
        ...
        if (options.follow) {
            ...
            while (isRunning) {
                ...
                try {
                    const { logs: newLogs, lastId: newLastId } = await fetchAndDisplayLogs(currentLastId);
