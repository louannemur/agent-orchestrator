...
export async function spawnCommand(taskId, options) {
    ...
    if (!selectedTaskId) {
        ...
        try {
            ...
            if (queuedTasks.length === 0) {
                ...
                console.log(chalk.dim("  Create a task first with: swarm task add \"title\"\n"));
                return;
            }
    ...
    if (count > 1) {
        ...
        for (const task of queuedTasks) {
            ...
            try {
                ...
                spinner.succeed(`Agent ${chalk.cyan(result.agentId.slice(0, 8))} → ${truncate(task.title, 40)}`);
                spawned++;
            }
            catch (error) {
