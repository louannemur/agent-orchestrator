...
export async function queueCommand() {
    ...
    try {
        ...
        // Summary
        console.log(`  ${chalk.yellow("Queued:")}      ${queue.queued} task${queue.queued !== 1 ? "s" : ""}`);
        console.log(`  ${chalk.blue("In Progress:")} ${queue.inProgress} task${queue.inProgress !== 1 ? "s" : ""}`);
        ...
        const inProgressTasks = queue.tasks.filter((t) => t.status === "IN_PROGRESS" || t.status === "VERIFYING");
        ...
        if (inProgressTasks.length > 0) {
            ...
            for (const task of inProgressTasks) {
                const agentInfo = task.assignedAgent
                    ...
                    : "";
                console.log(`    ${formatStatus(task.status.padEnd(11))} ${truncate(task.title, 45)}${agentInfo}`);
            }
        }
